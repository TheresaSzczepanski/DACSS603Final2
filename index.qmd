...
I considered additional explanatory vairables to control for the following:
...
## Fit 3 b): Explanatory: `MD Diff SD`, `WA Diff SD`, `MD Diff SD`
...
-   A school's percentage of students classified as
    ...
    is negatively associated with a school's variance in performance
    relative to students in the state on `Mathematics and Data` items,
    `SD MD Diff`, on a school's percentage of students `Exceeding or Meeting` expectations on the Introductory Physics MCAS.
:::
...
fit_md_size = lm(`EorM%` ~ (`MD Diff SD`  + `School Size` + `School Size`* `MD Diff SD`), data = HSPhy_NextGen_SchoolSum)
summary(fit_md_size)
AIC(fit_md_size)
BIC(fit_md_size)
...
* Linearity: our residuals plot appears to have a few outliers and a slighlty negative slope
...
Furthermore, when we remove those observations, and fit a new model, we find continue to be statistically significant with a 1 point increase in `MD Diff SD`: $p =6.62e-12$ associated with $-4.8152$ decrease in `EorM%`. 
...
This suggests that schools could benefit by spending time reviewing and supporting science teachers in implementing evidence based, best practices for the teaching of Mathematics within the Physics classroom.

2. Student performance as measured by `Reporting Cat%` was positively associated with the `Report_Cat_Weight`.

This suggests that schools could benefit by creating a scope and sequence for the calendar year that ensures that all content areas from the Massachusetts Curriculum framework are addressed within the academic year. There seems to be evidence to suggest that the items in the `Waves` category are not receiving enough instructional time within the school year as compared to those in the highest weighted `Motion and Forces` category.
